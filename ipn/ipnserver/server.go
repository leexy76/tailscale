...
func New(logf logger.Logf, logid string, pm *ipnlocal.ProfileManager, eng wgengine.Engine, dialer *tsdial.Dialer, opts Options) (*Server, error) {
	...
	_, profileKey := pm.CurrentProfile()
	if uid, ok := strs.CutPrefix(string(profileKey), "user-"); ok {
		u, err := lookupUserFromID(logf, uid)
		if err != nil {
			logf("ipnserver: found server mode auto-start key %q; failed to load user: %v", profileKey, err)
		} else {
			logf("ipnserver: found server mode auto-start key %q (user %s)", profileKey, u.Username)
			serverModeUser = u
		}
	}
